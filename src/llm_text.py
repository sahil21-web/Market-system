...
# Groq renames/retires models fairly often (llama-3.1-8b-instant and
# llama-3.3-70b-versatile were both shut down Aug 16, 2026) — so try a short
# list in order rather than betting everything on one exact name.
        ...
        for attempt in range(retries):
            try:
                ...
                if resp.status_code == 404:
                    # this model name no longer exists — try the next one
                    last_error = f"{model}: 404 not found"
                    break
                if resp.status_code in (429, 503):
                    time.sleep(5 * (attempt + 1))
                    last_error = f"{model}: {resp.status_code} on attempt {attempt + 1}"
                    continue
                ...
                time.sleep(3)
    return f"(AI research unavailable after trying all models: {last_error})"
